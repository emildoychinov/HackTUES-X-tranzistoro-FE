...
export let selectedMinHour: string = Object.keys(stat)[0];
  ...
  const updateChart = () => {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (Date.parse('11/01/2011'+ " "+ key) >= Date.parse('11/01/2011'+ " " +selectedMinHour) 
...
<Card>
  ...
  <div class="chip-container">
    ...
    {#each gym.departments as department}
      <Card on:click={() => handleSelection(department)} class={"scale-x-90 scale-y-90 rounded-3xl h-3 min-w-3 max-w-fit m-[1px] items-center justify-center text-black hover:bg-slate-400" + (selected === department ? " bg-slate-400 " : " bg-slate-300 ")}>
